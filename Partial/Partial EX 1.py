...
for i, j in stari_ascunse:
    ...
    start_prob =np.array([0.15, 0.15, 0.4, 0.15, 0.15, 0.15])
    transition_prob = np.array([ # probabilitatile de tranzitie intre stari
...
emission_prob = np.array([
        ...
        [0.15, 0.15, 0.4, 0.15, 0.15],
        [0.15, 0.15, 0.4, 0.15, 0.15],
        [0.15, 0.15, 0.4, 0.15, 0.15]   ])  # probabilitatilede observatie in functie de stare

# Modelul HMM
model = hmm.MultinomialHMM(n_components = numar_stari, startprob=start_prob, transmat=transition_prob)
model.emissionprob_ = emission_prob
hidden_states =model.predict(observatii_idx)



# Rulăm algoritmul Viterbi pentru secvența de observații
logprob, state_sequence = model.decode(observatii)

# Convertim secvența de stări în poziții din grid
drum = [idx_to_stare[idx] for idx in start_prob]
